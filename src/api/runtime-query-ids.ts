...
export interface RuntimeQueryIdsOptions {
  cachePath?: string;
  ttlMs?: number;
...
async function fetchText(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `HTTP ${response.status} for ${url}: ${body.slice(0, 120)}`
    );
  }
...
function resolveDefaultCachePath(): string {
  ...
  return path.join(homedir(), ".config", "xfeed", DEFAULT_CACHE_FILENAME);
}
...
function parseSnapshot(raw: unknown): RuntimeQueryIdSnapshot | null {
  ...
  const ttlMs =
    typeof record.ttlMs === "number" && Number.isFinite(record.ttlMs)
      ...
      : null;
  const ids =
    record.ids && typeof record.ids === "object"
...
async function readSnapshotFromDisk(
  cachePath: string
): Promise<RuntimeQueryIdSnapshot | null> {
  try {
...
export function createRuntimeQueryIdStore(
  options: RuntimeQueryIdsOptions = {}
): RuntimeQueryIdStore {
  ...
  const getSnapshotInfo =
    async (): Promise<RuntimeQueryIdSnapshotInfo | null> => {
